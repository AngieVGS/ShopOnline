...
public class ManagerPersistence {
	...
	public static ArrayList<Product> readProducts() throws FileNotFoundException {
		...
		for (JsonElement jsonElement : jsonArray) {
			listProducts.add(Shop.createProduct(jsonElement.getAsJsonObject().get("name").getAsString(), jsonElement.getAsJsonObject().get("price").getAsDouble(),jsonElement.getAsJsonObject().get("imagePath").getAsString(), jsonElement.getAsJsonObject().get("quantumAvailable").getAsInt(),Category.valueOf(jsonElement.getAsJsonObject().get("category").getAsString()), jsonElement.getAsJsonObject().get("percentageOfProfit").getAsDouble()));
		}
		return listProducts;
	}
	...
	public static ArrayList<Purchase> purchases() throws FileNotFoundException{
		...
		if (!(jsonArray.size()==0)) {
			for (JsonElement jsonElement : jsonArray) {
				for (JsonElement purchase : jsonElement.getAsJsonObject().get("listProductsPurchased").getAsJsonArray()) {
					if (jsonElement.getAsJsonObject().get("listProductsPurchased").getAsJsonArray().size()>0) {
						Product produ = Shop.createProduct(purchase.getAsJsonObject().get("product").getAsJsonObject().get("name").getAsString(), purchase.getAsJsonObject().get("product").getAsJsonObject().get("price").getAsDouble(),purchase.getAsJsonObject().get("product").getAsJsonObject().get("imagePath").getAsString(), purchase.getAsJsonObject().get("product").getAsJsonObject().get("quantumAvailable").getAsInt(),Category.valueOf(purchase.getAsJsonObject().get("product").getAsJsonObject().get("category").getAsString()), purchase.getAsJsonObject().get("product").getAsJsonObject().get("percentageOfProfit").getAsDouble());
	...
	public static ArrayList<User> readUsers() throws FileNotFoundException {
		...
		for (JsonElement jsonElement : jsonArray) {
			listUsers.add(Shop.createUser(jsonElement.getAsJsonObject().get("name").getAsString(), jsonElement.getAsJsonObject().get("password").getAsString()));
		}
		return listUsers;
	}
	...
	public static Administrator readAdministrator() throws FileNotFoundException {
		...
		return new Administrator(jsonObject.getAsJsonObject().get("id").getAsInt(),  jsonObject.getAsJsonObject().get("name").getAsString(), jsonObject.getAsJsonObject().get("password").getAsString());
	}
